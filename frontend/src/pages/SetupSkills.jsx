...
export default function SetupSkills() {
  ...
  useEffect(() => {
    api.get('/skills').then(({ data }) => setSkills(data.skills));
  ...
  const toggle = (setter, levels, setLevels, defaultLevel, id) => {
    setter((prev) => {
  ...
  const LevelEditor = ({ skills, selected, levels, onLevel }) =>
    skills.filter((s) => selected.has(s.id)).length > 0 && (
        ...
        {skills
          ...
          .map((skill) => (
            ...
            >
              {skill.name}
              <select
                value={levels[skill.id] || 'BEGINNER'}
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10 space-y-6">
      ...
      <div className={section}>
        ...
        <SkillPicker skills={skills} selected={offered} onToggle={(id) => toggle(setOffered, offeredLevels, setOfferedLevels, 'INTERMEDIATE', id)} />
        <LevelEditor skills={skills} selected={offered} levels={offeredLevels} onLevel={(id, l) => setLevel(setOfferedLevels, id, l)} />
      </div>
      ...
      <div className={section}>
        ...
        <SkillPicker skills={skills} selected={wanted} onToggle={(id) => toggle(setWanted, wantedLevels, setWantedLevels, 'BEGINNER', id)} />
        <LevelEditor skills={skills} selected={wanted} levels={wantedLevels} onLevel={(id, l) => setLevel(setWantedLevels, id, l)} />
      </div>
